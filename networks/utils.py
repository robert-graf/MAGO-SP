...
def ismap(x) -> TypeGuard[torch.Tensor]:
    if not isinstance(x, torch.Tensor):
        return False
    return (len(x.shape) == 4) and (x.shape[1] > 3)


def isimage(x) -> TypeGuard[torch.Tensor]:
    if not isinstance(x, torch.Tensor):
        return False
...
def instantiate_from_config(config: dict, **parms):
    if "config_path" in config:
        ckpt_path = None
        if "ckpt_path" in config:
            ckpt_path = config["ckpt_path"]
        config = OmegaConf.load(config["config_path"])["model"]  # type: ignore
        if ckpt_path is not None:
            if "params" not in config:
                config["params"] = {}
            config["params"]["ckpt_path"] = ckpt_path  # type: ignore
...
def parallel_data_prefetch(func: abc.Callable, data, n_proc, target_data_type="ndarray", cpu_intensive=True, use_worker_id=False):
    ...
    if isinstance(data, np.ndarray) and target_data_type == "list":
        ...
        if isinstance(data, dict):
            print('WARNING:"data" argument passed to parallel_data_prefetch is a dict: Using only its values and disregarding keys.')
