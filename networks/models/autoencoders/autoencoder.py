...
class Optimizer:
    def __init__(self, pl_module: pl.LightningModule, opt):
...
class VQModel(pl.LightningModule):
    def __init__(
        ...
        self.loss: Callable = instantiate_from_config(lossconfig)  # type: ignore
    ...
    def forward(self, input_, return_pred_indices=False):
        quant, diff, (_, _, ind) = self.encode(input_)
    ...
    def training_step(self, batch, batch_idx):  # noqa: ARG002
        ...
        opt_0, opt_1 = self.optimizers()  # type: ignore
    ...
    def _validation_step(self, batch, batch_idx, suffix=""):  # noqa: ARG002
        ...
        if version.parse(pl.__version__) >= version.parse("1.4.0"):  # type: ignore
    ...
    def configure_optimizers(self):
        ...
        if self.scheduler_config is not None:
            ...
            raise NotImplementedError("LambdaLR")
            scheduler = [
                {"scheduler": LambdaLR(opt_ae, lr_lambda=scheduler.schedule), "interval": "step", "frequency": 1},  # noqa: F821
                {"scheduler": LambdaLR(opt_disc, lr_lambda=scheduler.schedule), "interval": "step", "frequency": 1},  # noqa: F821
            ]
    ...
    def log_images(self, batch, only_inputs=False, plot_ema=False, **_kwargs):
        log = {}
        x = self.get_input(batch, self.image_key)
        x = x.to(self.device)
        if only_inputs:
...
class AutoencoderKL(pl.LightningModule):
    def __init__(
        ...
        if monitor is not None:
            self.monitor = monitor
        if ckpt_path is not None:
            print("Reload AutoencoderKL: ", ckpt_path)
            self.init_from_ckpt(ckpt_path, ignore_keys=ignore_keys)
    ...
    def forward(self, input_, sample_posterior=True):
        posterior = self.encode(input_)
    ...
    @torch.no_grad()
    def log_images(self, batch, only_inputs=False, **_kwargs):
        log = {}
        x = self.get_input(batch, self.image_key)
        x = x.to(self.device)
        if not only_inputs:
...
class IdentityFirstStage(torch.nn.Module):
    def __init__(self, *_args, vq_interface=False, **_kwargs):
        self.vq_interface = vq_interface  # TODO: Should be true by default but check to not break older stuff
        super().__init__()

    def encode(self, x, *_args, **_kwargs):
        return x

    def decode(self, x, *_args, **_kwargs):
        return x

    def quantize(self, x, *_args, **_kwargs):
