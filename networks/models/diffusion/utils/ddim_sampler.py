...
from networks.backbone.substructures.nd_layers import make_ddim_sampling_parameters, make_ddim_timesteps, noise_like


class DDIMSampler:
    def __init__(self, model, schedule="linear", **_kwargs):
        super().__init__()
        self.model = model
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0.0, verbose=True):
        ...
        sigmas_for_original_sampling_steps = ddim_eta * torch.sqrt(
            (1 - self.alphas_cumprod_prev) / (1 - self.alphas_cumprod) * (1 - self.alphas_cumprod / self.alphas_cumprod_prev)  # type: ignore
        )
    ...
    def ddim_sampling(
        ...
        iterator = tqdm(time_range, desc="DDIM Sampler", total=total_steps)  # type: ignore
    ...
    def p_sample_ddim(
        ...
        if score_corrector is not None:
            ...
            e_t = score_corrector.modify_score(self.model, e_t, x, t, condition, **corrector_kwargs)

        alphas = self.model.alphas_cumprod if use_original_steps else self.ddim_alphas  # type: ignore
        alphas_prev = self.model.alphas_cumprod_prev if use_original_steps else self.ddim_alphas_prev  # type: ignore
        sqrt_one_minus_alphas = self.model.sqrt_one_minus_alphas_cumprod if use_original_steps else self.ddim_sqrt_one_minus_alphas  # type: ignore
        sigmas = self.model.ddim_sigmas_for_original_num_steps if use_original_steps else self.ddim_sigmas  # type: ignore
        ...
        pred_x0 = clamp(pred_x0) if clamp is not None else pred_x0
