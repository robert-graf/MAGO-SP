...
from networks.models.diffusion.utils.distributions import DiagonalGaussianDistribution, normal_kl
from networks.utils import count_params, default, instantiate_from_config, isimage, ismap, log_txt_as_img, mean_flat


def disabled_train(self, mode=True):  # noqa: ARG001
...
class LatentDiffusion(DDPM):
    ...
    def get_learned_conditioning(self, c):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, "encode") and callable(self.cond_stage_model.encode):  # type: ignore
                c = self.cond_stage_model.encode(c)  # type: ignore
                if isinstance(c, DiagonalGaussianDistribution):
                    c = c.mode()
            else:
                c = self.cond_stage_model(c)  # type: ignore
        else:
    ...
    @torch.no_grad()
    def get_input(self, batch, k, encode=False):
        # return_first_stage_outputs=False, force_c_encode=False, cond_key=None, return_original_cond=False
    ...
    def encode_first_stage(self, x):
        if hasattr(self, "split_input_params"):
            if self.split_input_params["patch_distributed_vq"]:
                ks = self.split_input_params["ks"]  # eg. (128, 128)
                stride = self.split_input_params["stride"]  # eg. (64, 64)
                df_ = self.split_input_params["vqf"]
                ...
                fold, unfold, normalization, weighting = self.get_fold_unfold(x, ks, stride, df=df_)
    ...
    def shared_step(self, batch, **_kwargs):
        x = self.get_input(batch, self.first_stage_key, encode=True)
        c = self.get_input(batch, self.cond_stage_key)
        loss = self(x, c)
        return loss

    def forward(self, x_0_gt, cond, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x_0_gt.shape[0],), device=self.device).long()
        if self.conditioning_key is not None:
            assert cond is not None
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(cond)
                cond[self.conditioning_key] = c
            if self.shorten_cond_schedule:  # TODO: drop this option
                raise NotImplementedError("No longer supported")
                # tc = self.cond_ids[t].to(self.device)
                # cond = self.q_sample(x_start=cond, t=tc, noise=torch.randn_like(cond.float()))
        return self.p_losses(x_0_gt, cond, t, *args, **kwargs)
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if self.cond_stage_key in cond:
            ...
            elif self.conditioning_key == "crossattn" and "c_crossattn" not in cond:
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.cond_stage_key in ["image", "LR_image", "segmentation", "bbox_img"] and self.conditioning_key:  # TODO check for completeness
                ...
                patch_limits_tknzd = [
                    torch.LongTensor(self.bbox_tokenizer._crop_encoder(bbox))[None].to(self.device)  # type: ignore
                    for bbox in patch_limits  # type: ignore
            ...
            output_list = [self.forward_model(z_list[i], t, **cond_list[i]) for i in range(z.shape[-1])]
        ...
        x_recon = self.forward_model(x_noisy, t, **cond)
    ...
    def p_mean_variance(
        ...
        if clip_denoised:
            x_recon.clamp_(-1.0, 1.0)  # type: ignore
    ...
    def p_sample(
        ...
        if return_x0:
            model_mean, _, model_log_variance, x0 = outputs  # type: ignore
        else:
            model_mean, _, model_log_variance = outputs  # type: ignore
    ...
    def progressive_denoising(
        ...
        for i in iterator:
            ...
            if self.shorten_cond_schedule:
                assert self.conditioning_key != "hybrid"
                tc = self.cond_ids[ts].to(cond.device)  # type: ignore
                cond = self.q_sample(x_start=cond, t=tc, noise=torch.randn_like(cond))  # type: ignore
    ...
    def p_sample_loop(
        ...
        if start_T is not None:
            timesteps = min(timesteps, start_T)
        iterator = tqdm(reversed(range(timesteps)), desc="Sampling t", total=timesteps) if verbose else reversed(range(timesteps))
        ...
        if plot_diffusion_rows:
            ...
            for t in range(self.num_timesteps):
                if t % self.log_every_t == 0 or t == self.num_timesteps - 1:
                    t = repeat(torch.tensor([t]), "1 -> b", b=n_row)  # noqa: PLW2901
                    t = t.to(self.device).long()  # noqa: PLW2901
    ...
    def configure_optimizers(self):
        ...
        if self.cond_stage_trainable:
            ...
            params = params + list(self.cond_stage_model.parameters())  # type: ignore
        if self.learn_logvar:
            print("Diffusion model optimizing logvar")
            params.append(self.logvar)  # type: ignore
        opt = torch.optim.AdamW(params, lr=lr)
        if self.use_scheduler:
            assert "target" in self.scheduler_config  # type: ignore
            scheduler = instantiate_from_config(self.scheduler_config)  # type: ignore
