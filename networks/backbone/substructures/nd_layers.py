...
def make_beta_schedule(schedule, n_timestep, linear_start=1e-4, linear_end=2e-2, cosine_s=8e-3):
    if schedule == "linear":
        ...
        timesteps = timesteps / n_timestep + cosine_s
...
def make_ddim_timesteps(ddim_discr_method, num_ddim_timesteps, num_ddpm_timesteps, verbose=True):
    ...
    steps_out = np.concatenate([np.zeros((1,)), ddim_timesteps + 1, np.ones((1,)) * (num_ddpm_timesteps - 1)])
    if verbose:
        print(f"Selected timesteps for ddim sampler: {steps_out}, {len(steps_out)}")
    return steps_out
