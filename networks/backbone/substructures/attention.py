...
def default(val: T | None, d: T) -> T:
    ...
    return d() if isfunction(d) else d  # type: ignore
...
class FeedForward(nn.Module):
    def __init__(self, dim: int, dim_out: int | None = None, mult=4, glu=False, dropout=0.0):
        super().__init__()
        inner_dim = int(dim * mult)
        dim_out = default(dim_out, dim)  # type: ignore
