...
import torch
from networks.backbone.substructures.attention import SpatialTransformer,AttentionPool2d
from networks.backbone.substructures.nd_layers import avg_pool_nd, checkpoint, conv_nd, linear, normalization, timestep_embedding, zero_module
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        if use_new_attention_order:
            ...
            raise NotImplementedError("QKVAttentionLegacy")
...
class UNetModel(nn.Module):
    ...
    def __init__(  # noqa: C901
        ...
        if self.num_classes is not None:
            self.label_emb = nn.Embedding(num_classes, time_embed_dim)  # type: ignore
            ...
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(
                        ...
                        else SpatialTransformer(ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim)  # type: ignore
                    )
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                layers = [
                    ResBlock(
                        ch + ich, time_embed_dim, dropout, out_channels=model_channels * mult, dims=dims, use_checkpoint=use_checkpoint, use_scale_shift_norm=use_scale_shift_norm
                    )
                ]
                ...
                if ds in attention_resolutions:
                    ...
                    if legacy:
                        ...
                        dim_head = ch // num_heads if use_spatial_transformer else num_head_channels
                    layers.append(
                        AttentionBlock(ch, use_checkpoint=use_checkpoint, num_heads=num_heads_upsample, num_head_channels=dim_head, use_new_attention_order=use_new_attention_order)
                        if not use_spatial_transformer
                        else SpatialTransformer(ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim)  # type: ignore
                    )
    ...
    def forward(self, x, timesteps=None, context=None, y=None):  # **_kwargs
        ...
        :return: an [N x C x ...] Tensor of outputs.
        """
        # print("openaimodel.UNetModel", x.shape, timesteps.shape if timesteps is not None else timesteps, y.shape if y is not None else y) if self.first else None
        assert (y is not None) == (self.num_classes is not None), "must specify y if and only if the model is class-conditional"
        hs = []
        # print("openaimodel.UNetModel.timestep_embedding", timesteps.shape, self.model_channels) if self.first else None

        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)

        if self.num_classes is not None:
            assert y.shape == (x.shape[0],)  # type: ignore
            emb = emb + self.label_emb(y)

        h = x.type(self.dtype)
        for module in self.input_blocks:
            # print("openaimodel.UNetModel.input_blocks", h.shape, emb.shape, context.shape if context is not None else None) if self.first else None
            h = module(h, emb, context, first=self.first)
            hs.append(h)
        # print("openaimodel.UNetModel.middle_block", h.shape, emb.shape, context.shape if context is not None else None) if self.first else None
        h = self.middle_block(h, emb, context)
        for module in self.output_blocks:
            h = torch.cat([h, hs.pop()], dim=1)
            # print("openaimodel.UNetModel.output_blocks", h.shape, emb.shape, context.shape if context is not None else None) if self.first else None
...
class EncoderUNetModel(nn.Module):
    ...
    def forward(self, x, timesteps):
        ...
        if self.pool.startswith("spatial"):
            results.append(h.type(x.dtype).mean(dim=(2, 3)))
            h = torch.cat(results, axis=-1)  # type: ignore
            return self.out(h)
        else:
            h = h.type(x.dtype)
            return self.out(h)
